...
def set_form_value(browser, name, value, timeout=30, poll_frequency=0.5):
    """A smarter version of browser.fill with waiting for element to appear.

    Handles text, radio, checkbox, and select inputs with unified interface.
    Waits up to `timeout` seconds for the element to appear.
    """
    ...
    while time.time() < end_time:
        ...
        raise Exception(f"Element with name '{name}' not found after {timeout} seconds.")
